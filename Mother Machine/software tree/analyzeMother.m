%function [blobsGlobal] = analyzeMother(mask,rawVid)
    ...
    for k = 1:size(mask,3)
        ...
        blobsLocal = regionprops(mask(:,:,k),'Centroid');
    ...
    %% Get fluorescence data for each mother cell
    ...
    for k = 1:size(mask,3)
        ...
        for c = 1:numCells
            ...
            % Only look at large blobs user selected
            if totPix > 50
            
                % Loop through each pixel that's in the blob, c
                for pix = 1:totPix
                    ...
                    if pix == 1
                        ...
                        % CFP
                        blobsGlobal(c,6,k) = rawVid(y(pix),x(pix),k,1);
            
                        % GFP
                        blobsGlobal(c,7,k) = rawVid(y(pix),x(pix),k,2);
                    else
                        % CFP
                        blobsGlobal(c,6,k) = double(rawVid(y(pix),x(pix),k,1))+double(blobsGlobal(c,6,k));
            
                        % GFP
                        blobsGlobal(c,7,k) = double(rawVid(y(pix),x(pix),k,2))+double(blobsGlobal(c,7,k));
                    end
                end
                
                % Avg CFP
                blobsGlobal(c,6,k) = blobsGlobal(c,6,k)/totPix;
                % Avg GFP
                blobsGlobal(c,7,k) = blobsGlobal(c,7,k)/totPix;
                % Normalized GFP
                blobsGlobal(c,8,k) = double(blobsGlobal(c,7,k))/double(blobsGlobal(c,6,k));
            end
        end
    end
%end
