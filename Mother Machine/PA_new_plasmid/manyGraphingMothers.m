...
%%
plotL = blobsGlobal(:,:,1:204);
Lminus1 = zeros((size(plotL,3)-1)*size(plotL,1),1);
L = zeros((size(plotL,3)-1)*size(plotL,1),1);

count = 1;
for c = 1:size(plotL,1)
    for f = 2:size(plotL,3)
        L(count) = plotL(c,3,f);
        Lminus1(count) = plotL(c,3,f-1);
        count = count+1;
    end
end

figure(4)
histogram(log2(L./Lminus1),40);
hold on
y=ylim;
doubFac = log2(0.71);
plot([doubFac doubFac],[y(1) y(2)],'--k','LineWidth',3)
hold off
xlim([-2 2])
%histogram(L)
%xlim([0 12])
